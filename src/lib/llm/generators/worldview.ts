...
2. 完善是扩写而非缩减：在现有设定基础上继续丰富细节、补充血肉（如具体地名/势力名/境界名/规则细则），各字段篇幅不得少于原文（原文为空的字段除外），不要删并、压缩已有内容；
3. 修补各字段之间不自洽之处，仅删除与简介直接矛盾的设定；
4. 时代背景须贴合题材且多样化，避免千篇一律的末法/衰落设定；
5. 保留原有设定中有辨识度的亮点，不做推倒重写。
...
请按系统提示要求完善上述世界观，严格输出 JSON。`;
  ...
  const result = await chat(
    ...
    { responseFormat: 'json', temperature: 0.7, maxTokens: 2000 }
  );

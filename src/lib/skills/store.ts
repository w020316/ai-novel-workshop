...
const BUILTIN_SKILLS: Omit<WritingSkill, 'id' | 'enabled' | 'createdAt' | 'updatedAt' | 'builtin'>[] = [
  ...
  },
  {
    ...
    category: 'plot',
    source: 'builtin',
    sourceName: 'Swain「Scene & Sequel」场景技法',
    description: '场景=目标→冲突→灾难；续章=反应→困境→决定——让每章因果咬合不掉线。',
    instruction:
      '【场景与续章】\n把本章拆成一组「场景+续章」：\n一、场景三拍：视角人物带着具体目标进场（想要什么）→ 遭遇冲突（被谁/什么阻碍）→ 以"灾难"收场（目标受挫或付出代价，局面比进场时更糟）。目标模糊=场景失效，必须先补目标。\n二、续章三拍：灾难后先给情绪反应（身体与心理）→ 面对困境（可选项全是坏消息）→ 做出决定（引出下一场景的新目标）。\n三、连续两个场景都"顺利达成"视为节奏失效；连续三个"灾难"要给一次小胜回血，避免读者绝望弃书。',
  },
  {
    name: '英雄之旅十二段',
    category: 'outline',
    source: 'builtin',
    sourceName: '坎贝尔《千面英雄》/ 沃格勒改编',
    description: '普通世界→召唤→磨难→复活→携万能药归来，全书/整卷骨架一次搭好。',
    instruction:
      '【英雄之旅·结构骨架】\n用于全书或单卷规划，十二段按序：一、普通世界（展示主角的缺与痛）二、冒险召唤（金手指/事件打破日常）三、拒绝召唤（犹豫显真实）四、见导师（获得指引或信物）五、跨越第一道门（回不去了）六、考验·盟友·敌人（小试锋芒建关系网）七、接近最深的洞穴（战前压抑）八、磨难（生死考验，直面最大恐惧）九、奖赏（拿到关键收益）十、归途（被反扑/追杀，代价浮现）十一、复活（最终对决，完成蜕变）十二、携万能药归来（回归但有东西改变——能力/身份/认知）。\n对照检查：卡在哪一段，就补哪一段缺失的功能。',
  },
  {
    name: '救猫咪十五拍',
    category: 'outline',
    source: 'builtin',
    sourceName: 'Blake Snyder《Save the Cat》节拍表',
    description: '开场画面→催化剂→中点→灵魂黑夜→终场画面，精确到篇幅百分比的结构表。',
    instruction:
      '【救猫咪·十五拍】\n按篇幅百分比校验节奏：1 开场画面(1%)定基调；2 主题呈现(5%)借对话点题；3 铺垫(1-10%)展示"原来的样子"与不完美；4 催化剂(12%)打破日常的事件；5 争论(12-25%)要不要上船；6 进入第二幕(25%)主动选择进入新世界；7 B故事(30%)副线登场（常是情感线/盟友）；8 游戏时光(20-50%)兑现题材承诺的高光许诺；9 中点(50%)假胜利或假失败，赌注提高；10 反派逼近(50-75%)内外交困；11 一无所有(75%)失去最重要的东西；12 灵魂黑夜(75-80%)跌入谷底后的沉淀；13 进入第三幕(80%)找到解法；14 结局(80-99%)汇合所有线完成逆袭；15 终场画面(99%)与开场画面对照，证明改变发生。\n用途：某卷拖沓或失衡时，逐拍定位缺拍。',
  },
  {
    name: '冰山理论·留白',
    category: 'style',
    source: 'builtin',
    sourceName: '海明威冰山原则',
    description: '只写八分之一，八分之七沉在水下——删掉解释，让读者自己推断。',
    instruction:
      '【冰山写法】\n一、删解释：不写"他因为童年被抛弃所以不信任任何人"，只写他把递来的伞推开走进雨里。\n二、删情绪命名：情绪不下结论词，用可观察的动作、物件与沉默承载。\n三、删背景倾倒：往事不在当下场景整段倒出，拆成碎片随剧情自然露出。\n四、信任读者：关键信息给足线索即可，不重复强调；读者自己拼出的真相，比说破的震撼十倍。',
  },
  {
    name: '五感描写法',
    category: 'style',
    source: 'builtin',
    sourceName: '感官写作通识',
    description: '重要场景至少调动三种感官，用感官锚点替代抽象形容。',
    instruction:
      '【五感激活】\n一、重要场景入场时，在视觉之外至少再给两种感官：声（雨打屋檐）、嗅（血锈味/檀香）、触（砖石的凉）、味（嘴里的铁腥），各归到人物的具体感知，不做无主体的景物罗列。\n二、每个核心场景设一个「感官锚点」（一个反复出现的声音/气味/触感），在情绪转折处回收它。\n三、禁止堆砌五感清单逐项报菜名；感官必须与人物当下的注意力与情绪相关。',
  },
  {
    name: '情绪具象词典',
    category: 'style',
    source: 'builtin',
    sourceName: '《Emotion Thesaurus》情绪宝典思路',
    description: '身体信号+行为冲动+心理活动三层具象化，禁止直呼情绪名。',
    instruction:
      '【情绪具象化·三层】\n写情绪时按下述三层取信号组合，禁止直接写情绪名词（愤怒/害怕/感动）：\n一、身体信号：不受控的生理反应（耳根发热、指节发白、胃部下坠）。\n二、行为冲动：情绪引发的小动作（反复摩挲杯沿、把纸巾撕成条、笑得比哭难看）。\n三、心理活动：视角人物当下的念头（短句、破碎、可带自嘲）。\n规则：一次情绪最多取两三层中的各一两个信号，避免长篇生理流水账；重复场景换新信号，同一信号复用超过三次即失效。',
  },
  {
    name: '视角纪律',
    category: 'style',
    source: 'builtin',
    sourceName: 'POV 视角管理通识',
    description: '单章单一视角，只写视角人物能感知的；杜绝上帝视角突然剧透。',
    instruction:
      '【视角纪律】\n一、每章锁定一个视角人物，只写他看到的、听到的、知道的、感受到的；其他人的内心只能通过外在表现侧写。\n二、禁止视角漂移：不能写到一半突然钻进对手心里解释动机；若必须让读者知道对手心思，换对手视角另起一章/一节。\n三、视角即滤镜：同一事件，不同视角人物的解读应不同（偏爱的人看到偏爱的理由），用这层偏差制造信息差与误判张力。\n四、视角人物不在场的关键情节，通过转述/证据/后果带回，不直接演给读者看。',
  },
  {
    name: '千字一转·情节密度',
    category: 'plot',
    source: 'builtin',
    sourceName: '网文拆书高密度桥段思路',
    description: '每千字至少一次转折（信息/局势/关系），砍掉无功能段落。',
    instruction:
      '【情节密度自查】\n一、每千字自检：是否发生了至少一次"变化"——新信息揭晓、局势反转、关系变动、目标受挫或升级？没有即视为注水段。\n二、段落功能测试：删掉这段后，后文是否不成立？不成立=有功能，保留；无影响=删或合并。\n三、对话也要推进：每段对话结束时至少有一项与开始时不同（信息/立场/关系/力量对比），否则压缩成一句带过。\n四、转折不必都是大事件：一句错位的话、一个反常细节、一次误判都算——但必须有痕迹留在后文。',
  },
];

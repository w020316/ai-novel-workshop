...
- 当前线上真正可用：**智谱 GLM（glm-4-flash，主用）+ DeepSeek 槽位（agnes 网关 agnes-2.5-pro）**；**Gemini 已接入为可选 provider（OpenAI 兼容免费端点）**——当前网络连不上 Google 故未配 key，配置 `GEMINI_API_KEY` 后即自动升主；Qwen 未配置。
...
- **坑 6**：Gemini 主用需能连通 Google 申请免费 key；当前网络连不上 Google，故线上 GLM 为主。填 `GEMINI_API_KEY` 后由 `resolveProvider` 自动升主（无需改代码）。

...
**1. 「一句话灵感 → 自动开书」向导**（对标 AI-Novel-Writing-Assistant）✅ 已落地
- 方案：新增「灵感一句话」入口 → 调用 LLM 生成**开书包**（书名·题材·金手指·主线冲突·长线钩子·世界观种子）→ 预填或一键入项目。
- 价值：降低新手零成本起步；衔接既有「找灵感/拆书」入口。工作量中，纯前端编排 + 1 个生成器。
- 落地：新建项目页 `NewProjectClient` 一句话灵感 → `generateBookPackage` 开书包（含模板兜底）→ 一键预填三步向导。
...
**7. 世界时间线可视化**（对标 AI-Novel-Writing-Assistant）✅ 已落地
- 记忆管理页新增**时间线视图**（事件按卷序排列），一眼看全局事件流。
- 落地：记忆管理页 `WorldTimeline` 卡片（`src/components/memory/WorldTimeline.tsx`）。

### P3 · 后续可评估
- ~~整本生产 DAG 可视化 + 失败节点续跑（ai-story-builder）~~ ✅ 已落地：工作台「全书生产流水线」按卷分组节点网格 + 失败章红格定位跳转（DAG 边因章节线性依赖省略，见 docs/2026-09-05-全书生产流水线可视化.md）
- ~~任务队列后台常驻（配合批量续写）~~ ❌ 评估后不做：纯前端 + IndexedDB 架构无常驻进程，批量续写已有章级持久化 + 断点续跑 + 心跳互斥 + 自动重试，收益不成立

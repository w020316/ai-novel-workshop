...
> **进度注记（2026-09-02）**：P1、P2、P3、P4、P6 已实施并提交（`e8f21ae`、`3817c38`）。P1 规则扩至 11 类 + spot-fix 定点修复；P2 黄金三法则写入写作/剧情设计模板；P3 新增文风仿写指南注入写作链路；P4 爽点密度(Cooldown)已入健康体检；P6 用户手册补充质量保障与过审应对。**P5 扫榜/拆文仍为待实施的大版本增量**。

***

## 七、抖音视频补充调研（2026-09-02 追加·Openwrite）

> 依据用户提供的抖音视频「我不是码神」——《网文写作助手还能这么用？？？》（展示 AI 写作助手 Openwrite 被玩出的生态）。本视频信息密度较低，以生态/玩法观察为主，唯一具体可落地的功能点是「扫码拆书收集灵感」，与已规划但未实施的 P5 拆文高度重合。

### 7.1 视频要点

- **核心**：Openwrite 的 skill 机制催生「网文创作派 / skill 狂魔派 / 酒馆对话派」三大生态，并衍生短剧、编程、猫娘等分支——本质是工具的**可扩展性**与用户创造力，而非单一功能。

- **令我方可吸收的功能点**：「网文创作派利用基础功能 **扫码拆书收集灵感**」——把已上架书拆解成灵感/拆书素材，与我方计划的 **P5 拆文**（黄金三章 / 爽点密度 / 节奏拆解）同方向。

- **启示**：拆书收集灵感是网文作者的高频真实需求，且这类素材可直接作为「剧情/题材灵感库」反哺大纲与设定生成，技术含量集中在「参考书结构化拆解 + 灵感条目沉淀」，无需实时榜单抓取即可先落地「**粘贴拆书**」环节。

### 7.2 对 P5 拆文的边界修正（由本条视频引发）

- **范围收敛**：将 P5 从「扫榜（依赖平台实时榜单数据，前端难以可靠实现）」收敛为先做「**拆文分析 + 灵感沉淀**」——用户粘贴参考书章节/片段 → 拆解黄金三章/爽点密度/节奏/钩子/人设 → 生成可收藏的「拆书灵感卡」，并可并入大纲与设定。扫榜留作后续（需约定榜单数据源）。

- **与现有能力复用**：复用 `style/profile`（句长/对话/ngram）、`review/reader-review`（钩子/断章评分）、`health`（爽点密度），拆文分析器主要新增「结构化标签 + 灵感条目生成」。

**Source**：

- [我不是码神·网文写作助手还能这么用？？？（抖音）](https://v.douyin.com/2XLavPpJDMQ/)

- [Openwrite 生态（第 8-13 集关联视频片单）](https://www.douyin.com/video/7660190864156675391)

***

## 八、OpenWrite 深度调研（2026-09-02 追加·追查视频所述工具）

> 起因：用户要求「查一下他所用的网站/开源项目，完整使用一遍，参考借鉴并入」。视频所述为 **openxz.cn 的 OpenWrite**（网文写作助手）；另存同名开源实现 **LiPu-jpg/Openwrite**（Apache-2.0，本地优先长篇小说创作系统），一并核验。

### 8.1 产品级 OpenWrite（openxz.cn，桌面/移动 v1.3.5）

专为网文作者，核心特性可供参考：

| 特性                      | 说明                               | 与我方对照                           |
| ----------------------- | -------------------------------- | ------------------------------- |
| 风格蒸馏                    | 上传作品→AI 分析写作风格→生成可复用 Style Skill | ≈ 我方 **P3 文风仿写**（已实施 `3817c38`） |
| 小说拆解                    | 搜索网络小说或导入本地文件，一键拆解章节、获取灵感        | **P5'/未来实施**（推理拆文+灵感沉淀）         |
| 网文扫榜                    | 番茄/起点实时榜单，市场分析                   | P5 方向（依赖数据源，暂缓）                 |
| 起名工具                    | 人名/地名/功法/门派/兵器/法宝等 AI 生成         | **未实施，低成本高价值候选**                |
| 抽卡模式                    | 同时生成多个回复择优选                      | **未实施，低成本高价值候选**                |
| WebDAV 云同步、备忘录、多模型、网页搜索 | 基础设施类                            | 我方 Dexie 本地存储与多渠道模态已覆盖，不重复      |

### 8.2 开源实现 OpenWrite（LiPu-jpg/Openwrite，Apache-2.0）

本地优先的较长篇创作系统，与我校的「单章真源 + 运行态分离」架构同构，可借鉴（版权为思想、非源码）：

- **双 Agent 分工（Goethe 规划 / Dante 写作审查 + handoff）**：≈ 我方 plot-design + writing + consistency 编排，无需复制。

- **单人状态/时态批注**：`//**沈烬[位置]：贫民区工坊 -> 归墟港**`、`A~>B:敌意关系` 轻量内联批注记位置/伤势/认知/关系的时态变化，并做连续性冲突检查。→ **可为我方伏笔/角色看板补充「状态批注」方式**，低成本。

- **上下文预算渐进压缩**：`OPENWRITE_CONTEXT_TOKENS` 扣除输出预留/安全余量后按稳定优先级层层裁剪（旧章节记忆→大纲摘要→真相状态→人物数量→精确上文，最后硬适配），核心承诺/罗盘/当前章不删。→ **值得借鉴到 memory/assembler**，防止长文 token 超限。

- **37 维审稿闭环 + 修订提案 diff**：≈ 我方一致性 + 健康 + 冷读复核，仅维度更细，可后续增维度。

- **参考库证据化拆解、Style/Canon 人工确认采纳**：强力支持「不会自动污染项目」→ 与当前 P5' 一致。

### 8.3 凝练可并入项（P3+1 再排级）

我方对照后，可低成本并入的候选：

- **Q1（=P5'）粘贴拆文 + 灵感沉淀**：参考书章节 → 拆黄金三章/爽点/节奏/钩子，沉淀灵感卡反哺大纲设定。复用 style/reader-review/health。**直接立项，复用度高。**

- **Q2 起名工具**：按题材/风格生成人名/地名/功法/门派等，落地为一个设置页工具，成本低、网文作者高频。

- **Q3 抽卡模式**：章节生成时同参数并行出多个候选供择优选（适合非流式 bulk 场景，与控制成本权衡）。

- **Q4 角色状态批注 / 上下文预算压缩**：先做预算压缩（assembler 渐进裁剪）价值最稳定；状态批注需引入批注语法与索引，改动面大，后置。

**Source**：

- [OpenWrite 官网（openxz.cn）](http://openxz.cn/)

- [OpenWrite 开源实现（LiPu-jpg/Openwrite，Apache-2.0）](https://github.com/LiPu-jpg/Openwrite)

- [OpenWrite 另一开源实现（ilrein/openwrite，AGPL-3.0）](https://github.com/ilrein/openwrite)

- [OpenWriter 终端多 Agent 写作框架（KidOfJealous/OpenWriter，MIT）](https://github.com/KidOfJealous/OpenWriter)

...
- **待办（规划中，未实施）**：~~P5 扫榜~~（已落地：实时榜单抓取 6 平台 + 运行时查重）；~~实体图谱 Dashboard~~（已落地 2026-09-05：`src/lib/entity/graph.ts` + 数据看板 `EntityGraphCard`，含情节债务/伏笔追踪，见 `docs/2026-09-05-实体图谱Dashboard.md`）；~~4 视角多平台审稿~~（已落地：`src/lib/review/multi-platform-review.ts` 番茄/起点/知乎/冷读四视角 + 审校页接入）。数据看板/N1、模板速填/N2、设定迁移/N3 均已落地（见 §4）。

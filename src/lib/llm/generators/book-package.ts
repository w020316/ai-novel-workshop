...
要求：务求具体、可开写，避免空泛套话。
书名简介是全书优先级最高的资产（点击域 > 正文）：书名必须放大核心噱头、制造点击欲；简介用三句话写清「主角是谁/遭遇什么/要干什么」，并以悬念收尾——读者看完简介就知道非点开不可。简介写法三式任选其一：反差式（身份/处境的极端反差）、标签堆砌式（核心爽点标签高频直给）、情感认同式（戳中读者处境共鸣）；简介必须与书名卖点高度匹配。`;
